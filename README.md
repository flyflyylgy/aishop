...
| **个人资料** | 昵称/手机/头像修改，手机号唯一校验，PC/H5 双端个人资料页 |
| **忘记密码** | 用户名→6位验证码（Redis 10分钟）→验证码重置密码→吊销旧 token；重置后新密码登录、旧密码失效 |
...
- 前台功能：注册登录、首页（分类导航/搜索/热销榜）、商品详情、购物车（勾选/数量/清空）、收货地址簿（CRUD/默认地址/结算页选择）、个人资料修改、忘记密码（验证码重置）、结算下单（自动生成幂等键）、订单列表（状态筛选）、模拟支付闭环、确认收货、超时自动关单演示。
